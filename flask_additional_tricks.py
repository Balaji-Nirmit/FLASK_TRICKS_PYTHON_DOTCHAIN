...
def index():
  ...
  return render_template('index.html',commercial=commercial,homescreen=homescreen,latest=latest,editors_pick=editors_pick)
...
def dotchainsearch(dotchainsearch,pageno):
  if pageno!=0:
    ...
    return render_template('allblog.html',dotchainsearch=dotchainsearch,blogdata=blogdata,prev=prev,next=next,pageno=pageno)
...
def articles(pageno):
  ...
  if pageno!=0:
    ...
    return render_template('allblog.html',blogdata=blogdata,prev=prev,next=next,pageno=pageno)
    ...
    if request.method=='POST':
      ...
      try:
        ...
        if image:
            ...
            cursor.execute("INSERT INTO dotchainblogtable (BLOGID, TITLE, KEYWORD, CONTENT, CATEGORY, DESCRIPTION, USERNAME, DATE, OBJECTIVE, IMAGE, CODE) VALUES (?, ?, ?, ?, ?, ?, ?, strftime('%Y/%m/%d', 'now','localtime'), ?, ?, ?)",
               (request.form.get("article_id")+f"{str(time.time()).replace('.','_')}", request.form.get("title").lower(), request.form.get("keywords"), request.form.get("main_content"), request.form.get('category').lower(), request.form.get('description'), username, request.form.get('objective').lower(), filename, random.randint(1000,9999)))
      ...
      if request.method=='POST':
        ...
        if image.filename!="":
          ...
          if image:
            ...
            image_path = os.path.join(app.config['UPLOAD_FOLDER'], filename)
            image.save(image_path)
            cursor.execute("UPDATE dotchainblogtable SET TITLE=?,KEYWORD=?,CONTENT=?,CATEGORY=?,DESCRIPTION=?,DATE=strftime('%Y/%m/%d', 'now','localtime'),OBJECTIVE=?,IMAGE=?,CODE=? WHERE BLOGID=?",(request.form.get("title").lower(),request.form.get("keywords"),request.form.get("main_content"),request.form.get('category').lower(),request.form.get('description'),request.form.get('objective').lower(),filename,random.randint(1000,9999),blogid))  
            conn.commit()
            flash('updated and published with image')
            return redirect(url_for('admin',username=session['user']))
        else:
          cursor.execute('''UPDATE dotchainblogtable SET TITLE=?,KEYWORD=?,CONTENT=?,CATEGORY=?,DESCRIPTION=?,DATE=strftime('%Y/%m/%d', 'now','localtime'),OBJECTIVE=?,CODE=? WHERE BLOGID=?''',(request.form.get("title").lower(),request.form.get("keywords"),request.form.get("main_content"),request.form.get('category').lower(),request.form.get('description'),request.form.get('objective').lower(),random.randint(1000,9999),blogid))
          conn.commit()
...
def article(blogid):
  ...
  cursor.execute('SELECT * FROM dotchainblogtable WHERE BLOGID=(?)',(blogid,))
  blogdata=cursor.fetchone()
  cursor.execute('SELECT BLOGID,TITLE,CATEGORY,IMAGE,DATE FROM dotchainblogtable WHERE CATEGORY=(?) ORDER BY strftime("%m/%d/%Y", DATE) DESC LIMIT (?),(?)',(blogdata[4],random.randint(1,5),6))
...
def anime():
  ...
  return render_template('layout2.html',heading="Anime",headimage=['img/anime1.jpg','img/anime2.jpg'],blog=blog)
...
def gaming():
  ...
  return render_template('layout2.html',heading="Gaming",headimage=['img/gaming1.jpg','img/gaming2.jpg'],blog=blog)
...
def genz():
  ...
  return render_template('layout2.html',heading="Genz",headimage=['img/genz1.jpg','img/genz2.jpg'],blog=blog)
...
def tech():
  ...
  return render_template('layout2.html',heading="Tech",headimage=['img/tech1.jpg','img/tech2.jpg'],blog=blog)
